...
光线追踪（Ray Tracing）通过模拟光线传播来计算图像颜色：从相机向场景发射光线，计算与物体的交点，再根据材质决定反射/折射/散射并递归求色。  
本仓库当前实现是 **C++ CPU 递归光线追踪器**，核心对象包括 `ray`、`sphere`、`material`（`lambertian` / `metal` / `dielectric`）和 `camera`，最终输出 **PPM** 图像。
...
渲染时遍历每个像素，为每个像素生成多条带随机扰动的采样光线（抗锯齿）。  
相机由 `lookfrom/lookat/vup/vfov` 定义观察参数；当启用 `defocus_angle` 时，会在镜头圆盘上随机取样实现景深。

单像素的核心思路：

- 在像素区域内随机选取子像素坐标 \((u,v)\)
- 调用相机生成主光线 `r = cam.get_ray(u, v)`
- 对每条光线调用递归颜色函数并累加

### 3. 光线与物体求交

本仓库几何体以 **球体** 为主。将光线方程代入球方程可得到二次方程，判别是否有交点，并取合法区间内最近命中点：

\[
\|\mathbf{O}+t\mathbf{D}-\mathbf{C}\|^2=R^2
\]

命中记录（`hit_record`）通常包含：
...
- **Lambertian（漫反射）**：在法线半球随机散射，颜色由 albedo 衰减。
- **Metal（金属）**：按反射公式生成镜面方向，并用 `fuzz` 控制粗糙度。  
  \[
  \mathbf{R}=\mathbf{V}-2(\mathbf{V}\cdot\mathbf{N})\mathbf{N}
  \]
- **Dielectric（介质）**：基于折射率处理折射与反射，包含全内反射与 Schlick 近似反射率。

### 5. 递归追踪与最大递归深度

颜色计算函数会递归追踪散射光线：  
若命中物体，则由材质生成散射光线并返回“衰减 × 子光线颜色”；若未命中，则返回天空渐变背景。  
为避免无限递归，使用 `max_depth` 限制反弹次数；深度耗尽时返回黑色（无进一步贡献）。

### 6. 多重采样抗锯齿

每像素使用 `samples_per_pixel` 次随机采样：

- 降低几何边缘锯齿
- 平滑随机散射带来的噪声
- 通过平均值逼近像素真实辐照结果

采样数越高，噪声越低，但渲染耗时线性增加。

### 7. 景深（Depth of Field）

当 `defocus_angle > 0` 时，相机不再从单点发射，而是在镜头圆盘内随机选取光线起点，并指向焦平面。  
`focus_dist` 控制清晰平面距离；偏离焦平面的物体会出现虚化，从而得到更接近真实镜头的成像效果。
...
1. 累加所有采样得到的线性颜色
2. 除以采样数得到平均值
3. 做 Gamma 校正（常见为 \(\gamma=2.0\)，即近似开方）
4. 映射到 \([0,255]\) 并写入 PPM

Gamma 校正用于把线性空间亮度转换为更符合显示与人眼感知的输出。

### 9. 完整渲染流程（结合本仓库实现）

```text
构建场景（hittable_list + sphere + material）
    ↓
配置相机（分辨率、采样数、max_depth、视角、景深参数）
    ↓
遍历像素 (i, j)
    ↓
for s in [1..samples_per_pixel]:
    ...
    pixel += ray_color(r, depth=max_depth, world)
    ↓
...
本项目已经覆盖了入门级路径追踪核心链路：**相机采样 → 球体求交 → 材质散射/反射/折射 → 递归累积 → 抗锯齿与景深 → Gamma 校正与 PPM 输出**。  
在当前实现基础上，可继续扩展 BVH、更多几何体、纹理与更高级积分策略，以提升性能与真实感。
